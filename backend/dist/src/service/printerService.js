 ...
 */
function canvasToTsplBitmap(ctx, width, height) {
...
async function printReceipt(items) {
    ...
    try {
        ...
        if (!fs_1.default.existsSync(PRINTER_DEVICE)) {
        ...
        fd = fs_1.default.openSync(PRINTER_DEVICE, "w");
        // ส่งคำสั่งหัว
        for (const cmd of commands) {
            fs_1.default.writeSync(fd, cmd);
        }
        // ส่งข้อมูล Bitmap
        fs_1.default.writeSync(fd, bitmapData);
        // ส่งคำสั่งท้าย
